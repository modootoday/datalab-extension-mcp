...
describe("tools/list proxy", () => {
  ...
  /**
   * 🔴 Transport failures use the same vocabulary as a tool call, because the
   * two send the user somewhere different: a refused connection means the
   * connector is absent, a timeout means it is there and not answering.
   */
  it("🔴 surfaces a transport failure with the actionable vocabulary", async () => {
    const server = createAdapterServer({
