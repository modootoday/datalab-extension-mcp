/**
 * Adapter-to-daemon e2e over real HTTP against a mock daemon on an ephemeral
 * port. The adapter's handlers run through the real global fetch, so the
 * default fetch path and the JSON parsing are exercised end to end while only
 * the browser side is faked.
 */
