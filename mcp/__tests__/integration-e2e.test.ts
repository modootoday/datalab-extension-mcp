...
async function start(idleMs?: number, version?: string): Promise<Started> {
  ...
  // A home of its own. This boots the real daemon, and without a home named
  // here it keeps its state in the operator's — so the suite would read and
  // write whatever that machine happens to be holding.
  const home = await mkdtemp(join(tmpdir(), "mcp-e2e-home-"));
    ...
    try {
      ...
      if (res.ok) {
        ...
        return {
          ...
          exitCalls,
          // One key per machine: a browser presents the pairing token itself.
          browserToken: TOKEN,
        };
      }
    } catch {
...
async function attachPanel(
  ...
  const { base, browserToken } = daemon;
  ...
  const ack = (await handshake.json()) as { sessionId?: string };
  if (!handshake.ok || !ack.sessionId)
    throw new Error("panel handshake failed");
...
describe("whole-stack mock e2e: hosts ↔ adapters ↔ one daemon ↔ fake panel", () => {
  it(
    ...
    async () => {
      ...
      const { port } = daemon;
      const detach = await attachPanel(daemon);
  ...
  it(
    ...
    async () => {
      ...
      await waitFor(1);
      const detach = await attachPanel(daemon); // panel connects → re-fetch
      await waitFor(2);
