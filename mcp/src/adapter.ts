 ...
 *
 * The host config names an exact bootstrap version for deterministic startup.
 * The adapter then re-execs itself as the canonical build with the same argv:
 * same port, token and stdio, so the host keeps one continuous connector.
 *
...
export interface CliHandlers {
  ...
  /** What we ship and what is placed — reads only, runs no connector. */
  skills: (argv: readonly string[]) => Promise<void>;
  /**
...
export async function dispatchCli(
  ...
  if (sub === "skills") {
    await handlers.skills(argv.slice(3));
    return;
  }
