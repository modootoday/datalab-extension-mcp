...
2. **Paste this command and press Enter.** The saved command names an exact bootstrap version. At startup and while idle, the connector checks the canonical release and promotes itself to the current secure, compatible version. Diagnostic environments can opt out with `--no-self-update`.
   ...
   ```
   npx -y @modootoday/datalab-extension-mcp@1.9.1 install
   ```
...
## Choosing which writing skills are installed

Installing also places the bundled **writing skills**. An AI app reads the name and description of every installed skill on every start, so the more are installed the less room the conversation has. The default therefore places only the ones that work right away.

```
npx -y @modootoday/datalab-extension-mcp@1.9.1 skills list
npx -y @modootoday/datalab-extension-mcp@1.9.1 skills status
npx -y @modootoday/datalab-extension-mcp@1.9.1 skills attach <name>
npx -y @modootoday/datalab-extension-mcp@1.9.1 skills detach <name>
```
...
- Pass `--all-skills` to `install` for everything, or `--no-skills` for none.

**Three are held back by default** — two need the photo or video editor to be **already open**, and one is AI photo generation, which only works inside the extension panel. Attach them if you use those editors.
...
There are 188 tools. Sending all of them as a list runs into the cap each AI app
...
1. Run this in a terminal (it removes the entries registered in your AI apps).
   ...
   ```
   npx -y @modootoday/datalab-extension-mcp@1.9.1 uninstall
   ```
