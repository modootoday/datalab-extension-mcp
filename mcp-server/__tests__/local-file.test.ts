/**
 * The connector accepts bounded regular image bytes from one stable handle.
 */
import { mkdtemp, rename, writeFile, rm, symlink } from "node:fs/promises";
...
describe("openLocalImage", () => {
  ...
  it("rejects symbolic links", async () => {
    const path = join(dir, "linked.png");
    await symlink(join(dir, "photo.png"), path);
    const out = await openLocalImage(path);
    expect(out.ok).toBe(false);
    if (!out.ok) expect(out.reason).toBe("not_a_file");
  });

  it("streams from the same file handle that was validated", async () => {
    const path = join(dir, "swap.png");
    const original = Buffer.concat([PNG_HEAD, Buffer.from("first")]);
    await writeFile(path, original);
    const out = await openLocalImage(path);
    expect(out.ok).toBe(true);
    if (!out.ok) return;

    await rename(path, join(dir, "swap-original.png"));
    await writeFile(path, Buffer.concat([PNG_HEAD, Buffer.from("second")]));
    const chunks: Buffer[] = [];
    for await (const chunk of out.stream) chunks.push(Buffer.from(chunk));
