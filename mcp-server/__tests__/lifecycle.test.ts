/**
 * Idle lifecycle against a fake clock. The properties: an idle-exit fires only
 * after the full window at zero refs, any activity before expiry cancels it,
 * and a held ref keeps the daemon alive however much time passes.
 */
import { describe, expect, it, vi } from "vitest";

import { Lifecycle, type TimerHandle } from "../src/lifecycle.js";

/** A scheduler driven by hand: advancing fires everything then due. */
function fakeTimers() {
  let seq = 0;
  let clock = 0;
