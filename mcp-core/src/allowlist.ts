...
/**
 * Tool tiers — the enum stays closed; a tier only decides how the user is
 * asked. Read runs always, write costs one session consent, destructive and
 * paid are confirmed per call, and a closed tier is never named to a host.
 *
 * 🔴 Destructive and paid must never fall under session consent: a toggle
 * switched on once and forgotten cannot be what deletes something or spends
 * money. A name in neither this table nor the read-only list is refused.
 */
...
export const TOOL_TIERS: Readonly<Record<string, ToolTier>> = Object.freeze({
  ...
  // capability editor_insert_image carries. A session toggle would open the
  // whole filesystem once and stay open.
  ...
  // than overwriting it. The day it gains a replace fallback, a destructive act
  // hides under a session toggle the user granted once — so that fallback is a
  // review-block, not a convenience.
  editor_place_draft: "write",
  // Opening a window edits no document, but it does create a tab in the user's
  // browser, and a session that has not enabled editing can do nothing with the
  // window anyway — one tier keeps it one thing for the user to allow.
  editor_open_window: "write",
  // 🔴 Closing a window cannot be undone and the user's draft may be inside, so
  // it is asked per call rather than covered by a forgotten session toggle.
...
export type TierPolicy = "always" | "session" | "per-call" | "closed";
