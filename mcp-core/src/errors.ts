...
export const MCP_TOOL_REASONS = [
  ...
  "invalid_args",
  /** More than one browser is active, so a mutation needs an explicit target. */
  "browser_required",
  /** The tool itself failed. */
  "tool_error",
  /** The upstream API says the requested account or resource does not exist. */
  "upstream_not_found",
...
export const MCP_TRANSPORT_REASONS = [
  ...
  "unauthorized",
  /** The panel that was answering went away mid-call. */
  "disconnected",
  /** Another panel took the bridge over mid-call. */
  "superseded",
  /** No registered browser owns the supplied id. */
  "browser_unknown",
  /** The registered browser has no active stream. */
  "browser_detached",
] as const;
...
export const REASON_POLICY: Readonly<Record<McpReason, ReasonPolicy>> =
  Object.freeze({
    ...
    /**
     * Retryable where not_connected is not, and the difference is who has to
     * move: these two are a panel handing the bridge to the next one, which
     * needs nobody. If it really left, the retry lands on not_connected and
     * stops there — one extra hop to reach the right answer.
     */
    disconnected: Object.freeze({ retryable: true }),
    superseded: Object.freeze({ retryable: true }),
    browser_detached: Object.freeze({
      retryable: true,
      retryAfterMs: 1_000,
    }),
    ...
    browser_required: Object.freeze({ retryable: false }),
    tool_closed: Object.freeze({ retryable: false }),
    missing_in_build: Object.freeze({ retryable: false }),
    upstream_not_found: Object.freeze({ retryable: false }),
    ...
    browser_unknown: Object.freeze({ retryable: false }),
