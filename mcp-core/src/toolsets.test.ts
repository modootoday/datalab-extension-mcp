...
import { NATIVE_TOOLSET, buildToolsets, toolsetOf } from "./toolsets.js";
...
describe("NATIVE_TOOLSET", () => {
  it("실재하는 묶음을 가리킨다", () => {
    const sets = buildToolsets(["my_daily_brief", "my_realtime"]);
    expect(sets.some((s) => s.slug === NATIVE_TOOLSET)).toBe(true);
  });
});
