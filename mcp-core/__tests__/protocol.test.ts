/**
 * Freeze-floor tests, not ordinary schema tests. Every shipped binary lives
 * on a user's disk forever, so the properties below ARE the compatibility
 * contract: a failure here means a change that breaks handshakes with binaries
 * we cannot reach.
 */
...
describe("hello ABI — permanent, additive-only", () => {
  it("parses a hello that predates the capabilities field", () => {
    // The shape already-shipped panels send. It must parse forever.
    const parsed = HelloSchema.parse(GOOD_HELLO);
    expect(parsed.capabilities).toEqual([]);
  });

  it("strips unknown fields instead of rejecting them", () => {
    // The other half of additive evolution: an old schema receiving a new
    // field must ignore it, which .strict() anywhere in the handshake would
    // revoke.
  ...
  it("carries the extension id when sent, and leaves it undefined otherwise", () => {
    // Additive: a panel predating the field omits it, and a newer one sends
    // its own id so the server can identify it without an Origin header.
...
describe("version constants", () => {
  ...
  it("minSupported server version is a plain semver", () => {
    // The panel compares against this value verbatim, so anything but a
    // plain semver poisons every comparison downstream.
...
describe("classifyFrame — unknown is not an error", () => {
  ...
  it("classifies a frame type from the future as unknown, not malformed", () => {
    // A new panel pushes a frame type invented after this binary shipped.
...
describe("no .strict() in the handshake path", () => {
  it("every handshake schema strips rather than rejects", () => {
    // Asserted per schema so a future .strict() on any one of them fails
