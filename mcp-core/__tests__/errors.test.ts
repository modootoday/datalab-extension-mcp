...
describe("REASON_POLICY", () => {
  ...
  // A panel handing the bridge to the next one needs nobody to act, so it is
  // retryable — while having no panel at all needs a person and is not. Reading
  // the first as the second told hosts to give up on an ordinary reconnect.
  it("separates a panel changing hands from having no panel", () => {
    expect(REASON_POLICY.disconnected.retryable).toBe(true);
    expect(REASON_POLICY.superseded.retryable).toBe(true);
    expect(REASON_POLICY.not_connected.retryable).toBe(false);
  });
});
