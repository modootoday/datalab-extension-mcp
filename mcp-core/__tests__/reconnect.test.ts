...
describe("backoffDelay", () => {
  // Randomness is injected so the bounds are assertable rather than flaky.
  const lo = () => 0;
  const hi = () => 1;
  ...
  // The peer is a local process the user just started, so a minute of silence
  ...
  it("honors an explicit cap (P2-1 two-tier)", () => {
    // With the tight offline cap, even a high attempt tops out fast.
    for (const attempt of [10, 50, 1000]) {
      expect(backoffDelay(attempt, hi, RECONNECT_OFFLINE_CAP_MS)).toBe(
        RECONNECT_OFFLINE_CAP_MS,
      );
    }
    // Longer, so a refusing server is not hammered.
...
describe("shouldRetry", () => {
  // A server that is not running is the normal state, not a fault.
  ...
  // Retrying cannot fix a version mismatch, so it is noise until a human acts.
  ...
  it("stops until the browser is registered", () => {
    expect(
      shouldRetry({
        kind: "error",
        reason: "browser_registration_required",
        message: "",
        nextRetryInMs: 1000,
      }),
    ).toBe(false);
...
describe("isUsable", () => {
  it("is true only when connected", () => {
    ...
    expect(isUsable({ kind: "standby" })).toBe(false);
...
describe("describeStatus", () => {
  ...
  // Folding these together would tell someone whose panel is a follower to
  // start their AI app, which changes nothing — the panel that would answer is
  // in another window.
  it("does not describe standby the way it describes off", () => {
    expect(describeStatus({ kind: "standby" })).not.toBe(
      describeStatus({ kind: "idle" }),
    );
  });
  ...
  // An error already carries an actionable sentence, and re-wording it here
  // would drop the detail that names the fix.
