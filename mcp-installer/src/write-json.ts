/**
 * The hygiene floor for every direct config write.  Read immediately before
 * writing, since host apps rewrite their own configs while running; refuse
 * outright when the existing file does not parse; merge only our own key and
 * preserve every other byte; back up first; write a sibling temp file and
 * rename (a cross-device rename is not atomic); re-read to verify and restore
 * the backup if it fails. Expected refusals are reported, never thrown.
 */
...
/** UTC, fixed width — rotation relies on it sorting lexicographically. */
...
async function readDoc(io: Io, filePath: string): Promise<JsonDocState> {
  ...
  try {
    ...
    if (!isPlainObject(parsed)) {
      // A top-level array or scalar is valid JSON but not a config document we
      // understand, and merging into it would destroy it.
...
/**
 * Copy the current bytes aside, then prune all but the newest BACKUP_KEEP.
 * Runs before the mutating write, so a crash mid-write still leaves the
...
export async function restoreNewestBackup(
  io: Io,
  filePath: string,
): Promise<boolean> {
...
/**
 * Install path: merge our own server key into the file. When the file does not
 * exist yet, a fresh minimal document is created, with no backup because there
...
export async function upsertServerKey(
  ...
  if (state.parseError) {
    // Never write over a file we could not read, and never leave a backup
    // of a refusal: a refused host leaves the directory untouched.
  ...
  if (existingServers === undefined) {
    ...
    // The key exists but is not an object, which is the same refusal class as
    // a parse failure: merging would destroy someone's data.
    return { ok: false, changed: false, reason: "parse" };
  }
...
/**
 * Uninstall path: delete only our own server key.  An emptied container is
 * left in place, because removing it would touch a key we do not own.
 */
