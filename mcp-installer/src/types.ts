...
export interface Io {
  platform: string;
  /**
   * Loopback HTTP, for the one step that talks to a running connector. Part of
   * the seam so a test never reaches a real port — on a developer's machine
   * that port may hold a connector this would then end.
   */
  fetch?: typeof fetch;
  homedir(): string;
  ...
  /**
   * Removes an EMPTY directory, and throws when it is not.
   *
   * That failure is the feature: a skill directory holding a file the user
   * added is one we must leave, and refusing to recurse is how the rule
   * "only what our marker lists" survives contact with directories.
   */
...
export interface RunOptions {
  ...
  hosts?: string[];
  /** Place bundled skills as well. Default on; --no-skills turns it off. */
  skills?: boolean;
  /**
   * Place the conditional ones too. Default off — see run-skills for what is
   * held back and why the listing budget makes that the default.
   */
  allSkills?: boolean;
  /** Add these optional skills without an interactive selection. */
  skillSlugs?: string[];
}
...
export interface HostResult {
  ...
  backupPath?: string;
  /**
   * Which surface this row is about.  One table, not two — a host can appear
   * once for its MCP config and once for its skills, and the reader needs to
   * see both under the same name rather than in a second list.
   */
  surface?: "mcp" | "skills";
}
