...
function printDetected(io: Io, detected: DetectedHost[]): void {
  printBody(
    (line) => io.out(line),
    `${detected.length}개 프로그램을 찾았어요.`,
  );
  for (const detectedHost of detected) {
    printItem(
      (line) => io.out(line),
      detectedHost.displayName,
      tierLabel(detectedHost.tier),
    );
  }
}
...
async function offerCliInstall(io: Io): Promise<InstallableCli | null> {
  ...
  INSTALLABLE_CLIS.forEach((cli, i) => {
    printItem((line) => io.out(line), `${i + 1}. ${cli.displayName}`);
  });
  printItem((line) => io.out(line), CLI_OFFER_SKIP_LABEL);
...
export async function runInstall(opts: RunOptions, io: Io): Promise<number> {
  ...
  printStep((line) => io.out(line), 1, 5, "연결 키 확인");
  ...
  printBody((line) => io.out(line), "인증키를 확인했어요.");
  ...
  printStep((line) => io.out(line), 2, 5, "연결할 AI 프로그램 찾기");
  printBody((line) => io.out(line), "설치된 프로그램을 확인하는 중이에요…");
  ...
  printStep((line) => io.out(line), 3, 5, "스킬 선택");
  const skillRoot = skillsPayloadRoot();
  let skillPlan: Awaited<ReturnType<typeof chooseSkillSelection>> = null;
  if (opts.skills === false) {
    printBody((line) => io.out(line), "스킬 설치를 건너뜁니다.");
  } else {
    const staged = await readPayloads(io, skillRoot);
    if (staged === null || staged.length === 0) {
      printBody(
        (line) => io.out(line),
        "이 설치 파일에는 스킬이 없어 연결 프로그램만 설치합니다.",
      );
    } else {
      skillPlan = await chooseSkillSelection(io, staged, {
        all: opts.allSkills === true,
        requested: opts.skillSlugs,
        interactive: opts.yes !== true && io.isInteractive(),
      });
      if (skillPlan === null) {
      ...
      printBody(
        (line) => io.out(line),
        `추천 ${skillPlan.defaultCount}개 + 선택 ${skillPlan.selected.size}개 = ` +
          `${skillPlan.total}개를 설치합니다.`,
      );
    }
  }
  ...
  if (actionable.length > 0 && opts.yes !== true) {
    const answer = await io.ask(
      installQuestion(actionable.length, skillPlan?.total),
    );
  ...
  printStep((line) => io.out(line), 4, 5, "연결하는 중");
  ...
  if (opts.skills !== false) {
    results.push(
      ...(await placeSkills(
        ...
        opts.version,
        skillPlan?.selected ?? false,
      )),
    );
  }

  printStep((line) => io.out(line), 5, 5, "결과");
