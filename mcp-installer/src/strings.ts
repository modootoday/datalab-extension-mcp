...
/** Final confirmation after program detection and skill selection. */
export function installQuestion(count: number, skills?: number): string {
  return skills === undefined
    ? `위 ${count}개 프로그램에 연결할까요?`
    : `위 ${count}개 프로그램에 연결하고 스킬 ${skills}개를 설치할까요?`;
}
