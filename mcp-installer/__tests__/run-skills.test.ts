...
describe("기본집합", () => {
  const many = {
    [`${PAYLOAD}/datalab-tone-manner/SKILL.md`]:
      "---\nmetadata:\n  install: default\n---\na",
    [`${PAYLOAD}/datalab-card-news/SKILL.md`]:
      "---\nmetadata:\n  install: optional\n---\nb",
    [`${PAYLOAD}/datalab-video-script/SKILL.md`]:
      "---\nmetadata:\n  install: optional\n---\nc",
    [`${PAYLOAD}/datalab-photo-prompt/SKILL.md`]:
      "---\nmetadata:\n  install: optional\n---\nd",
  };
  const mem = () => createMemIo({ platform: "linux", home: HOME, files: many });
  const optionalSlugs = async (m: ReturnType<typeof mem>) =>
    ((await readPayloads(m.io, PAYLOAD)) ?? [])
      .filter((payload) => payload.install === "optional")
  ...
  it("조건부 3종은 기본 설치에서 빠진다", async () => {
    ...
    for (const held of await optionalSlugs(m)) {
  ...
  it("설치 중 번호로 여러 선택 스킬을 고른다", async () => {
    const m = createMemIo({
      platform: "linux",
      home: HOME,
      files: many,
    ...
    const payloads = (await readPayloads(m.io, PAYLOAD)) ?? [];
    const plan = await chooseSkillSelection(m.io, payloads, {
      interactive: true,
    });
    expect([...plan!.selected]).toEqual([
      "datalab-card-news",
      "datalab-photo-prompt",
    ]);
    expect(plan?.total).toBe(3);
    expect(m.prompts.join("\n")).toContain("Enter = 건너뛰기");
  });

  it("Enter 는 추천 구성만, all 은 전체 구성이다", async () => {
    const recommended = createMemIo({
      platform: "linux",
      home: HOME,
      files: many,
      prompts: [""],
    });
    const payloads = (await readPayloads(recommended.io, PAYLOAD)) ?? [];
    expect(
      (
        await chooseSkillSelection(recommended.io, payloads, {
          interactive: true,
        })
      )?.total,
    ).toBe(1);

    const all = createMemIo({
      platform: "linux",
      home: HOME,
      files: many,
      prompts: ["all"],
    ...
    expect(
      (await chooseSkillSelection(all.io, payloads, { interactive: true }))
        ?.total,
    ).toBe(4);
  });

  it("비대화형 --skill 선택과 잘못된 이름을 구분한다", async () => {
    const m = mem();
    const payloads = (await readPayloads(m.io, PAYLOAD)) ?? [];
    const chosen = await chooseSkillSelection(m.io, payloads, {
      requested: ["datalab-card-news"],
    });
    expect([...chosen!.selected]).toEqual(["datalab-card-news"]);
    expect(
      await chooseSkillSelection(m.io, payloads, { requested: ["missing"] }),
    ).toBeNull();
    expect(m.out.join("\n")).toContain("선택할 수 없는 스킬: missing");
  });

  it("실제 적재물의 선택 설치 메타데이터를 읽는다", async () => {
    ...
    let payloads = [] as NonNullable<Awaited<ReturnType<typeof readPayloads>>>;
    for (const root of candidates) {
      payloads = (await readPayloads(io, root)) ?? [];
      if (payloads.length > 0) break;
    }
    expect(payloads.length).toBeGreaterThan(0);
    expect(
      payloads.filter((payload) => payload.install === "default"),
    ).toHaveLength(9);
    const optional = payloads.filter(
      (payload) => payload.install === "optional",
    );
    const includesPanelOnly = payloads.some(
      (payload) => payload.slug === "datalab-photo-prompt",
    );
    expect(optional).toHaveLength(includesPanelOnly ? 11 : 10);
    expect(optional.map((payload) => payload.slug)).toEqual(
      expect.arrayContaining([
        "datalab-ad-budget-review",
        "datalab-commerce-health",
        "datalab-shopping-demand",
        "datalab-comment-reaction",
      ]),
    );
  });
});
