/**
 * The hygiene floor, byte-asserted against a real filesystem rather than a
 * mock, because the point is what actually lands on the user's disk: foreign
 * keys preserved, backups rotated, temp files cleaned up, and a refusal
 * leaving nothing new behind.
 */
...
describe("upsertServerKey", () => {
  it("adds only our key, preserves unknown keys, backs up, leaves no temp file", async () => {
    ...
    // Exactly the target plus one backup: the temp file left nothing behind.
    ...
    const { io } = createTempIo({
      ...
      overrides: {
        async writeFile(path: string, content: string): Promise<void> {
          // Only the first temp write, the mutation, is corrupted; the backup
          // before it and the restore after it pass through untouched.
...
describe("unparseable tier-2 config through the full install flow", () => {
  it("refuses to write, leaves zero new files, reports failure with a snippet", async () => {
    ...
    // 🔴 A refused write leaves the bytes untouched and no new files at all.
...
describe("Cursor url entry — real filesystem", () => {
  ...
  it("merges a url entry, preserving unrelated keys byte-for-byte", async () => {
    ...
    await fs.mkdir(dir);
    const target = join(dir, "mcp.json");
    // A pre-existing config with a foreign server the user set up.
    const before = {
    ...
    // 🔴 Both the foreign server and the unrelated key are untouched.
