...
describe("TOML host (Codex config without the CLI)", () => {
  ...
  /**
   * The desktop app ships no CLI, so this used to end in "paste it yourself"
   * for anyone who had the app and not the command. The config is now merged:
   * only our own table is written, and the user's keys keep their bytes.
   */
  it("merges into an existing config.toml and keeps the user's keys", async () => {
    const codexDir = join(home, ".codex");
    await fs.mkdir(codexDir, { recursive: true });
    ...
    expect(after).toContain('approval = "never"');
    expect(after).toContain("[mcp_servers.datalab]");
    expect(after).toContain(`DATALAB_MCP_TOKEN = "${VALID_TOKEN}"`);
    // The original bytes are recoverable, as with every other direct write.
    expect((await fs.readdir(codexDir)).some((n) => n.includes("backup"))).toBe(
      true,
    );
    expect(out.join("\n")).toContain("설정 파일을 고쳤어요");
  });

  /**
   * A shape the text merge cannot locate is still refused, and refusing means
   * the file is left exactly as found with the snippet printed instead. A
   * mangled config would be the false success this package exists to avoid.
   */
  it("refuses an inline-table config and falls back to the snippet", async () => {
    const codexDir = join(home, ".codex");
    await fs.mkdir(codexDir, { recursive: true });
    const tomlPath = join(codexDir, "config.toml");
