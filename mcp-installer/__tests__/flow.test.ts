...
describe("install flow", () => {
  ...
  it("단계·목록·결과가 같은 텍스트 골격을 쓴다", async () => {
    const harness = twoHostSetup();
    ...
    expect(harness.out.some((line) => line.startsWith("    - "))).toBe(false);
    expect(harness.out.filter((line) => line.includes("Claude Code"))).toEqual(
      expect.arrayContaining(["  - Claude Code", "  - [성공] Claude Code"]),
    );
  });
  ...
  it("ends a successful install with the byte-exact restart notice", async () => {
    ...
    expect(code).toBe(0);
    // Pinned as a literal: this is a frozen copy contract.
