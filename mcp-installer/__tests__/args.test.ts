/**
 * CLI argv construction and input validation.  The exact arrays are
 * contracts: a missing user scope silently binds the server to whatever
 * directory the command was pasted in, so flags are pinned element by element.
 */
...
describe("file entry", () => {
  ...
  it("gives Cursor a url entry (HTTP MCP direct), not an adapter command", () => {
    ...
    // No adapter to spawn and no secret on the line: the daemon holds the token.
...
describe("input validation happens before any io", () => {
  async function expectRefused(
    ...
    expect(code).toBe(1);
    // The refusal lands before any detection or write. The banner prints
    // first but is neither, so the message is asserted by presence rather than
    // by position.
